...
export function PlayerTable({
  ...
  return <>
                ...
                <td className="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                  <div className="flex items-center">
                    <div className="h-10 w-10 flex-shrink-0">
                      {player.profileImage ? <img className="h-10 w-10 rounded-full object-cover" src={player.profileImage} alt="" /> : <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold">
                          {player.firstName[0]}
                          {player.lastName[0]}
                        </div>}
                    </div>
      ...
      setPlayerToDelete(null);
    }} onConfirm={handleDeleteConfirm} title="Remove Player" message={playerToDelete ? `Are you sure you want to remove ${playerToDelete.firstName} ${playerToDelete.lastName} from the squad? This action cannot be undone.` : ''} confirmText="Remove Player" variant="danger" />
    </>;
}