...
import { useDeleteCoachMutation } from '../store/api/endpoints/coachApi';
...
export function CoachCard({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative group h-full">
      ...
      <ConfirmDialog isOpen={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)} onConfirm={handleDeleteConfirm} title="Remove Coach" message={`Are you sure you want to remove ${coach.full_name} from the coaching staff? This action cannot be undone.`} confirmText="Remove Coach" variant="danger" />
    </div>
  );
}