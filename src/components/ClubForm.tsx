...
import { useCreateClubMutation } from '../store/api/endpoints/clubApi';
export function ClubForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (name.trim().length < 2) {
      ...
      toast.error('Club name must be at least 2 characters long');
      return;
    }
    ...
    try {
      const response = await createClub({
        name: name.trim(),
        club_logo: imageurl || undefined,
        address: address.trim() || undefined,
        Lga: lga.trim() || undefined,
        contact_information: contact.trim() || undefined,
        club_status: homeOrAway || undefined
      });

      if (response) {
        toast.success(`Club "${name}" registered successfully`);
        navigate('/');
      } else {
